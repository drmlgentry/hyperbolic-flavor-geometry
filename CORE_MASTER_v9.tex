...
\paragraph{Geometric selection of the flavor manifolds.}
A systematic survey of all 12 closed orientable hyperbolic 3-manifolds
with $H_1=\mathbb{Z}/5$ in the census establishes that exactly one
manifold simultaneously satisfies all hard geometric constraints:
(i)~Lucas-pure covering tower; (ii)~contains a geodesic of length
$\tfrac{3}{2}\log\phi$ (the $\sigma_\mathrm{opt}$ geodesic);
(iii)~smallest volume among all closed hyperbolic 3-manifolds.
That manifold is $M_\mathrm{PMNS} = m003(-2,3)$. Among Lucas-pure
$\mathbb{Z}/5$ manifolds, $M_\mathrm{CKM} = m006(-5,2)$ achieves
the lowest Gaussian kernel fitness to the PDG CKM matrix (0.016482).
No other manifold satisfies all constraints for either sector.
\emph{Note on the PMNS fitness value:} The Borel construction
fitness 0.005087 is the global minimum of the QR map relative to
the PDG PMNS matrix, reachable by any manifold given sufficient
optimization. The geometric selection of $M_\mathrm{PMNS}$ rests
on constraints (i)--(iii), not on the fitness value alone.
...
Lucas numbers: $n = 18 \approx L_6$, $n = 29 \approx L_7$,
