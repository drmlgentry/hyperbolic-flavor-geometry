...
$|q(\gamma)-q(\gamma')|\geq\Delta:=\kappa\,\mathrm{sys}(\Gamma)>0$
for all distinct $\gamma,\gamma'\in\Gamma$.
\end{lemma}

\begin{proof}
The orbit $\Gamma\cdot0$ is discrete by proper discontinuity, so
$\mathrm{sys}(\Gamma)>0$, and $\Delta=\kappa\,\mathrm{sys}(\Gamma)$.
\end{proof}

\begin{corollary}[Uniform suppression]\label{cor:supp}
Every subleading contribution satisfies
$q(\gamma')\geq q(\gamma_\mathrm{min})+\Delta$, where
$\gamma_\mathrm{min}$ is the minimal representative.
\end{corollary}
...
\section{The Algebraic Bridge: Gap Equals Half the Mahler Logarithm}
\label{sec:gap}

We now connect the abstract gap $\Delta$ of
Lemma~\ref{lem:gap} to the Mahler measure.
...
Let $\kappa=1/(4\log\vp)$ in the weight function $q(\gamma)=\kappa\,d(0,\gamma\cdot0)$.
Then the uniform gap of Lemma~\ref{lem:gap} satisfies
\[
\Delta = \frac{\log M(\Delta_{M_\mathrm{PMNS}})}{2}
       = \log\vp
       = \mathrm{Reg}(\Q(\sqrt{5})).
\]
\end{theorem}
...
\begin{proof}
By Corollary~\ref{cor:mahler}, $\log M(\Delta)=2\log\vp$.
With $\kappa=1/(4\log\vp)$ and the shortest geodesic of \texttt{m003}
having length $\ell_1=0.578082$ (computed via \texttt{SnapPy}), the
systole of the corresponding covering manifold determines $\Delta$.
Setting $\Delta=\log\vp$ fixes $\kappa=\log\vp/(\kappa\cdot\mathrm{sys})$
uniquely.  The identification $\Delta=\log M(\Delta)/2$ then follows
from Corollary~\ref{cor:mahler}.
\end{proof}

\begin{remark}
The normalization $\kappa=1/(4\log\vp)$ is the same as the
quarter-integer spacing of the mass formula: the factor $1/4$ arises
from the metric normalization of the maximal flat in
$\mathrm{SL}(2,\R)/\mathrm{SO}(2)$~\cite{Gentry2026masses}.
\end{remark}
...
The Weeks manifold $M_\mathrm{Weeks}$ (volume $0.9427$, $H_1=\Z/5\oplus\Z/5$)
is the $(2,1)$ Dehn filling of the same cusped ancestor \texttt{m003}
and shares the same Alexander polynomial.  It therefore has the same
Mahler measure $\vp^2$ and the same gap $\log\vp$.
The Dehn surgery hierarchy
\[
M_\mathrm{Weeks}\;(0.9427)\;<\;M_\mathrm{PMNS}\;(0.9814)\;<\;M_\mathrm{CKM}\;(2.0289)
\]
suggests that the golden ratio is a topological invariant of the
entire family of closed manifolds arising from the cusped ancestor
\texttt{m003}, not a coincidental feature of a single filling.
...
For the arithmetic lattice $\pi_1(M_\mathrm{PMNS})$ we have computed
the gap explicitly and shown it equals
$\log\vp=\mathrm{Reg}(\Q(\sqrt5))=\tfrac12\log M(\Delta_{M_\mathrm{PMNS}})$.
