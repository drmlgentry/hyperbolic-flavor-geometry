...
in any load-bearing step. This field is distinct from the Eisenstein
field $\Q(\sqrt{-3})$ of the cusped parent $m003$ (Section~\ref{sec:homology}).

Third, a fixed-protocol short-word atlas over thirteen
...
$M$ is of minimal volume within the $H_1 \cong \Zfive$ stratum of the
closed orientable census, not of minimal volume overall; the global
minimum is the Weeks manifold, of volume $0.942707\ldots$, with
$H_1 \cong \Zfive \oplus \Zfive$ \cite{GMM2009}. The invariant trace
field $\kinv(M)$ established below (Section~\ref{sec:itf}) is a
quartic field distinct from the Eisenstein field $\Q(\sqrt{-3})$ of
the cusped parent $m003$.
...
We use the standard word notation $A = a^{-1}$ and $B = b^{-1}$. We
fix throughout the two-generator presentation of the cusped
...
\begin{remark}
The eliminant $E(T)$ and $X^4+X^3-1$ are minimal polynomials of
different generators of the same quartic field $K_{283}$; the fields
are isomorphic and the discriminant and signature agree.
\end{remark}
...
$\tr(A^2) = x^2 - 2$, so $x^2 - 2 \in \kinv(M)$ and hence
$\Q(x^2-2) \subseteq \kinv(M)$. Since $\Q(x^2-2) = \Q(x^2)$, it suffices
to compute the degree of $x^2$. Exact elimination of
$x$ between the quartic and $S - x^2 = 0$ shows that the minimal
polynomial of $x^2$ has degree four. Hence
$[\Q(x^2):\Q] = 4$, and since $\Q(x^2) \subseteq K_{283}$ (as $x \in A_{\mathrm{fill}} \cong K_{283}$) with both of
degree four,
\begin{equation}
...
\subsection{The short-word atlas}

We computed a fixed structural atlas of $m003$ and thirteen of its
Dehn fillings. The
...
Let $X = X_{\SLtwo}(m003)$ in the Fricke coordinates above, let
$I_X = I(X) \subset \Q[x,y,z]$ denote its vanishing ideal, let $X_0$
...
\end{equation}
an intersection of three primes; that is, $V(J_D) = X_0 \cup L \cup N$
with $L = V(y+1,\,x+2z)$ and $N = V(y-2,\,x-z)$ two affine lines, each
...
\end{equation}
set-theoretically: neither $L$ nor $N$ lies on the relator variety,
...
gives $N$. The quotients $\Q[x,x^{-1}]$ and $\Q[t]$ are domains, so
...
and $N$ meet the relator variety only along $L \cap X_0$,
$N \cap X_0$, and the zero-dimensional relator components do not
...
determines \emph{all} such coincidences, for words of every length. Recall
the line $N = V(x-z,\ y-2)$ of Proposition~\ref{prop:jd}; its ideal is
...
it are Horowitz/Southcott's, not new here. Ginzburg--Rudnick \cite{GinzburgRudnick1998} give an
...
Proposition~\ref{prop:ipc} needs only a point $(c,2,c)$ on a character
subvariety $X$ with $c=\lambda+\lambda^{-1}$, $\lambda$ not a root of
unity; we have verified this for the geometric components of $m003$
and $m006$ (Remark~\ref{rem:m006}) but not for any other manifold or
component. Characterizing which components of which manifolds'
character varieties meet the abelian specialization line $N$ at a
non-cyclotomic point --- and hence enjoy the same collision-ideal
closure $J_\infty=J_L$ for some finite $L$ --- is open.
...
Short-word atlas &\texttt{m003\_structure\_atlas.sage} & computed \\
