...
the cusped parent $m003$. The lepton-mixing construction of
Section~\ref{sec:borel} is logically downstream of the exact results
in Sections~\ref{sec:homology}--\ref{sec:atlas}: the matched-procedure
null calibration of Section~\ref{sec:null} finds no evidence of
statistical exceptionality, and none of the exact results depend on
that construction in either direction.
...
\item \textbf{Generalizing the intersection criterion.} Proposition~\ref{prop:ipc}
needs only a point $(c,2,c)$ on a character subvariety $X$ with
$c=\lambda+\lambda^{-1}$, $\lambda$ not a root of unity; we have verified
this for the geometric components of $m003$ and $m006$
(Remark~\ref{rem:m006}) but not for any other manifold or component.
Characterizing which components of which manifolds' character varieties
meet the abelian specialization line $N$ at a non-cyclotomic point ---
and hence enjoy the same collision-ideal closure $J_\infty=J_L$ for some
finite $L$ --- is open.
