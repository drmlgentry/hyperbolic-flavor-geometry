...
\cite{Horowitz1972}, who also studies when distinct words have the same
character under all representations into $\mathrm{SL}_2$. Southcott
\cite{Southcott1979} shows that trace polynomials fall into $2^n$
classes according to the exponent sums of the word modulo $2$. The
hypotheses here differ in kind: equality is assumed only on a
subvariety $X$ (not for all representations), and the conclusion concerns
the absolute exponent sum $|e_a|$ rather than its parity. We make no
priority claim.
\end{remark}
