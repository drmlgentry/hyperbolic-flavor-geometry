...
$\tr\rho_c(w)=\tr\rho_c(a)^{e_a(w)}$. The Cayley--Hamilton relation
$A^{k+1}=cA^k-A^{k-1}$ and $\tr A^{-1}=\tr A$ give
$\tr A^k=S_{|k|}(c)$. Thus $\tau_w(c,2,c)=S_{|e_a(w)|}(c)$ for all
$c$, hence as polynomials. Since $S_n$ is monic of degree $n$ for
...
\label{prop:ipc}
Let $p=(c,2,c)\in N$ with $c=\lambda+\lambda^{-1}$, and suppose $\lambda$
is not a root of unity. If $w,v$ are words with $\tau_w(p)=\tau_v(p)$
--- in particular if $\tau_w=\tau_v$ on any set containing $p$ --- then
$|e_a(w)|=|e_a(v)|$ and $\Delta_{w,v}\in I(N)$.
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:nexp}, $\tau_w(p)=S_n(c)$ and $\tau_v(p)=S_m(c)$ with
$n=|e_a(w)|$, $m=|e_a(v)|$, and $S_k(c)=\lambda^k+\lambda^{-k}$ (induction
on $k$). For $n,m\ge0$,
\begin{equation}
S_n(c)-S_m(c)=(\lambda^n-\lambda^m)\bigl(1-\lambda^{-n-m}\bigr).
\end{equation}
If this vanishes and $n\ne m$, then $\lambda^{n-m}=1$ or
$\lambda^{n+m}=1$ with a nonzero exponent, so $\lambda$ is a root of
unity, a contradiction. Hence $n=m$, and Lemma~\ref{lem:nexp} gives
$\Delta_{w,v}\in I(N)$.
\end{proof}
