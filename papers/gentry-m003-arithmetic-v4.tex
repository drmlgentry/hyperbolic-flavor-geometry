...
\begin{proof}
Abelianizing the presentation, the relator $r$ contributes the
relation determined by its exponent vector, giving
$H_1(m003) \cong \Z \oplus \Zfive$ with the $\Zfive$ torsion generated
by the class of $a$. The peripheral curves $\mu, \lambda$ have
abelianized images with the stated coefficients, so filling along
$(p,q)$ adjoins the relation $p[\mu] + q[\lambda] = 0$. Evaluating the
resulting $2 \times 2$ integer presentation matrix and taking Smith
normal form gives order $5|2p+q|$.
\end{proof}
