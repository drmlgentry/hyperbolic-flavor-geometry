...
Degree 5 yielded exactly one cover, with the H₁ and volume shown
...
\item The cover H₁ is \emph{always} $(\ZZ/p_n)^2$ — exactly
...
\begin{observation}[Primality and cover H₁ through $n=10$]
Table~\ref{tab:primality} lists $p_n$, its factorisation,
the cover H₁, and primality through $n=10$.

\begin{table}[htbp]
\centering
\caption{Values $p_n=5n(n+1)+1$, factorisation,
cover H₁, and primality.}
\label{tab:primality}
\begin{tabular}{ccclc}
\toprule
...
The cover H₁ is $(\ZZ/p_n)^2$ in all cases, including
