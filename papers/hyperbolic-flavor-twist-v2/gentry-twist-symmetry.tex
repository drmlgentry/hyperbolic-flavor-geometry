...
The census enrichment factor is $1.24\times$ over random baseline
...
$\theta_{12}^q$ & $13.04\degr$ & aB & $\MCKM$ & $12.75\degr$ & $2.2\%$ \\
...
$\lambda = s(2-s)$.
Using the length spectrum up to $\ell = 5$ (approximately 200 primitive
geodesics, where a primitive geodesic is one not a proper power of another),
we evaluate $Z_M(s)$ numerically on the critical line $\mathrm{Re}(s) = 1$
and locate the first zero by sign changes of $\mathrm{Re}[Z_M(1+it)]$,
giving $\lambda_1 = 1\cdot(2-1) + t_1^2/(...)$; more precisely,
$\lambda_1 = s_1(2-s_1)$ where $s_1 = 1 + ir_1$ and $r_1$ is the first
positive zero of the real part. These values should be regarded as
computational estimates; a rigorous determination would require a
complete length spectrum and certified zero-finding.
