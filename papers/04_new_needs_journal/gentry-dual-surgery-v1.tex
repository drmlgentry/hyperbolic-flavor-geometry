...
$\Z/2$, while those of \mfld{m019} and \mfld{m006} (the cusped
parent of the companion manifold $M_{\mathrm{CKM}}$) have Galois
groups $S_4$ and $S_3$. We prove that the $\mfld{m003}$ field is
linearly disjoint from each of the latter two, with Galois closures
giving direct-product groups
\[
\Z/2\times S_4 \quad\text{and}\quad \Z/2\times S_3,
\]
of orders 48 and 12. Disjointness in each case follows from the
distinct quadratic fields $\Q(\sqrt{-3})$, $\Q(\sqrt{-283})$, and
$\Q(\sqrt{-59})$ occurring in the respective Galois closures --
conceptual arguments, not merely computational identifications. We
also report a negative result: a systematic search over 9 candidate
manifolds with $S_4$-type cusp fields, at Dehn surgery slopes
$|p|,|q|\le 15$, finds no dual filling description of
$M_{\mathrm{CKM}}$ analogous to the $M_{\mathrm{PMNS}}$ case. We
note, without asserting a physical derivation, that these groups
admit the corresponding Weyl-group identifications
$\Z/2\times S_4\cong\Weyl(\SU(2))\times\Weyl(\SU(4))$ and
$\Z/2\times S_3\cong\Weyl(\SU(2))\times\Weyl(\SU(3))$.
All computations are verified by SnapPy and Sage at 300-bit
precision.
\end{abstract}

\subjclass[2020]{Primary 57K32; Secondary 11R32, 57K31}

\keywords{hyperbolic 3-manifolds, Dehn surgery, Meyerhoff manifold,
Galois groups, trace fields, linear disjointness}
...
\item $M_{\mathrm{CKM}} = \mfld{m006}(-5,2)$ is the quark mixing
      manifold.
\item $K_M$ denotes the cusp field of a one-cusped manifold $M$,
      computed from its cusp shape. We do not assert this equals
      the invariant trace field; that equality is not established
      here for any of the manifolds below.
...
$K_2=\Q[x]/(x^4-x-1)$ (Proposition~\ref{prop:m019}, degree 4), with
Galois closure $K_2^{\rm gal}$ of degree 24 and group $S_4$. Since
$S_4$ has a unique subgroup of index 2 (namely $A_4$, as
$S_4^{\rm ab}\cong\Z/2$), $K_2^{\rm gal}$ has a unique quadratic
subfield, namely $\Q(\sqrt{\disc(x^4-x-1)})=\Q(\sqrt{-283})$, since
$\disc(x^4-x-1)=-283$. Because $-283/-3=283/3$ is not the square of
a rational, $\Q(\sqrt{-283})\ne\Q(\sqrt{-3})$: the two quadratic
fields differ, so $K_1$ and $K_2^{\rm gal}$ share no common
subfield beyond $\Q$ and are therefore linearly disjoint. Their
compositum has Galois group $\Z/2\times S_4$, order 48.

This is independently confirmed by direct computation in Sage: the
compositum $K_1K_2$, computed via \texttt{composite\_fields}, has
degree 8 over $\Q$ (consistent with $8=2\times4$), and the Galois
closure $L$ of $K_1K_2$ has degree 48 with $\Gal(L/\Q)$ of GAP
small-group id $[48,48]$, the same id as an independently
constructed $S_4\times\Z/2$
(\texttt{SymmetricGroup(4)}$\times$\texttt{CyclicPermutationGroup(2)}).
\end{proof}
...
(degree 3, defined by $x^3+2x+1$). The Galois closure $K_3^{\rm
gal}$ has degree 6 and Galois group $S_3$; its unique quadratic
subfield is $\Q(\sqrt{\disc(x^3+2x+1)}) = \Q(\sqrt{-59})$, since
$\disc(x^3+2x+1)=-59$. Because $-59/-3=59/3$ is not the square of
a rational, $\Q(\sqrt{-59})\ne\Q(\sqrt{-3})$: the two quadratic
fields differ, so $K_1$ and $K_3^{\rm gal}$ share no common
subfield beyond $\Q$ and are therefore linearly disjoint. Their
compositum has Galois group $\Z/2\times S_3$, order 12, confirmed
independently by Sage's \texttt{galois\_group()}, which returns
GAP small-group id $[12,4]$ -- the id shared by $\Z/2\times S_3$
itself, consistent with (not merely asserted by) the conceptual
argument above.
\end{proof}

\begin{remark}
This result therefore \emph{does} extend the Pati--Salam correspondence of
Corollary~\ref{cor:group} to a third gauge factor: the Galois
closure of $K_{\mfld{m003}}\cdot K_{\mfld{m006}}$ has group
$S_3\times\Z/2\cong W(SU(3))\times\Z/2$, mirroring the
$S_4\times\Z/2$ structure of Corollary~\ref{cor:group} with the
same $\Z/2$ factor contributed by $K_{\mfld{m003}}$.
\end{remark}
...
Two questions remain explicitly open and are not addressed by this
paper. A third point, on linear disjointness with a third field,
is resolved below and in fact extends rather than limits the
correspondence:
...
\item \textbf{Linear disjointness with a third field, resolved.}
Proposition~\ref{prop:d6} shows that $K_{\mfld{m003}}=\Q(\sqrt{-3})$
and the Galois closure of $K_{\mfld{m006}}$ (unique quadratic
subfield $\Q(\sqrt{-59})\ne\Q(\sqrt{-3})$, from
$\disc(x^3+2x+1)=-59$) are linearly disjoint over $\Q$, with
compositum Galois group $\Z/2\times S_3\cong\Z/2\times\Weyl(\SU(3))$,
order 12. This result therefore \emph{does} extend
the Pati--Salam correspondence of Corollary~\ref{cor:group} to a
third gauge factor, with the same $\Z/2$ contributed by
$K_{\mfld{m003}}$ pairing with $\Weyl(\SU(4))$ in one compositum
and $\Weyl(\SU(3))$ in the other.
\end{itemize}
...
\url{reproduce/verify_s4_manifold_table.py}), archived as of commit
\texttt{a29be9fa19aae0224a1e9d3cc2b42e48c8b618f7}.
