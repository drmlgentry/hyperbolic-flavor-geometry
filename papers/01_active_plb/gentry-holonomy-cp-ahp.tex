...
\subjclass[2020]{Primary 57K32; Secondary 20H10}

\keywords{hyperbolic 3-manifolds, Kleinian groups, flat $U(1)$ connections,
CP violation, orientation-reversing isometries, holonomy}
...
\begin{remark}[Conjugacy in $\pi_1(M)$ versus abelianization]
\label{rem:conjugacy}
The condition $\Theta_*[\gamma] = [\gamma]$ in the theorem above
refers to conjugacy in $\pi_1(M)$, not in its abelianization
$H_1(M;\mathbb{Z})$. For the Meyerhoff manifold, $\Theta_*$ acts by
inversion $k \mapsto -k$ on $H_1(M;\mathbb{Z}) = \mathbb{Z}/5$,
sending a generator $\gamma$ to $\gamma^{-1}$ in the abelianization.
In the abelian quotient $[\gamma] \neq [\gamma^{-1}]$. However, the
theorem operates in $\pi_1(M)$ via the holonomy representation.

The holonomy $\rho \colon \pi_1(M) \to \mathrm{PSL}(2,\mathbb{C})$
is faithful for hyperbolic 3-manifolds, and two elements are conjugate
in $\pi_1(M)$ if and only if their images are conjugate in
$\mathrm{PSL}(2,\mathbb{C})$. For any $A \in \mathrm{SL}(2,\mathbb{C})$,
the Cayley--Hamilton theorem gives $A^{-1} = \mathrm{tr}(A)\cdot I - A$,
hence $\mathrm{tr}(A^{-1}) = \mathrm{tr}(A)$. Since loxodromic conjugacy
classes in $\mathrm{PSL}(2,\mathbb{C})$ are determined by trace,
$\rho(\gamma)$ and $\rho(\gamma^{-1})$ are always conjugate in
$\mathrm{PSL}(2,\mathbb{C})$. Therefore $\gamma$ and $\gamma^{-1}$
are conjugate in $\pi_1(M)$, the inversion map $\Theta_*$ preserves
$[\gamma]$, and the hypothesis of the theorem is satisfied.

Explicitly: $W_\rho(\gamma) = e^{2\pi i/5}$. Under CP, the transformed
holonomy is $W_{\rho^\Theta}(\gamma) = \overline{W_\rho(\gamma^{-1})}
= \overline{e^{-2\pi i/5}} = e^{2\pi i/5}$. The absolute value is
unchanged, but $\arg W_\rho(\gamma) = +2\pi/5$ maps to $-2\pi/5$ in
the CP-transformed frame. The sign flip of the argument is the CP-odd
observable.
\end{remark}
...
and proofs are the author's own work.
...
\bibitem{Gentry:CKM}
M.~L. Gentry,
\newblock Quark Mixing from Hyperbolic Geometry: The CKM Matrix from a Compact Arithmetic 3-Manifold,
\newblock SSRN~\href{https://doi.org/10.2139/ssrn.6583550}{6583550} (2026).

\bibitem{Gentry:PMNS}
M.~L. Gentry,
\newblock Lepton Mixing from Borel Structure of Hyperbolic Holonomy,
\newblock SSRN~\href{https://doi.org/10.2139/ssrn.6583553}{6583553} (2026).
\end{thebibliography}
