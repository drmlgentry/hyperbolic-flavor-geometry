...
minimum within the scanned word ensemble (words up to length~5,
$\sigma\in[0.1,2.0]$), with $p<10^{-4}$ against
...
Third, the cusped ancestor of $\MPMNS$ has invariant trace field
$\KPMNS = \mathbb{Q}(\sqrt{-3})$ (imaginary quadratic);
the closed manifold has a degree-4 ITF with discriminant $-283$~\cite{Chinburg98}.
The charged lepton masses are Eisenstein norms in $\Zom = \mathbb{Z}[\omega]$.
(Note: this is the ITF of the cusped parent $m003$;
the closed manifold $\MPMNS$ has a degree-4 ITF with
discriminant $-283$~\cite{Chinburg98}.
The Eisenstein norm encoding uses the cusped parent ring.)
...
CP violation, in contrast to the CKM manifold's trace field
(a degree-10 field with signature $(8,1)$, predominantly real).
\end{abstract}

\subjclass[2020]{Primary 57K32; Secondary 11R32}

\keywords{hyperbolic 3-manifolds, PMNS matrix, lepton mixing, CP violation,
trace fields, Meyerhoff manifold, Borel decomposition}
...
\item \textbf{Quarter-Lucas geodesic.} $\MPMNS$ contains a geodesic
  of length $\sigma_{\mathrm{opt}}=\frac{3}{2}\log\sqrt{13/5}=0.7218$
  (second shortest, to
...
\item The invariant trace field of $M_{\mathrm{CKM}}$
  is a degree-10 field with signature $(8,1)$ (8 real places,
  1 complex pair), confirmed by SnapPy at 500-bit precision.
  The predominantly real character of this field suppresses
  quark CP violation ($J_{\mathrm{CKM}}=0$ at leading order).
\end{enumerate}
The signature of the trace field — $(0,1)$ purely imaginary
for $\KPMNS$ versus $(8,1)$ predominantly real for $K_{\mathrm{CKM}}$
— determines whether holonomy elements are loxodromic or hyperbolic.
\end{theorem}
...
\end{itemize}
The trace field signature dichotomy --- $(0,1)$ for the lepton
sector versus $(8,1)$ for the quark sector ---
