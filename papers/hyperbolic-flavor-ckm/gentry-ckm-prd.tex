...
to within $0.4\%$ with a single free parameter ($\sigma = 0.49$).
An improved word triple $\{\texttt{abbAbbaB}, \texttt{Ab}, \texttt{B}\}$
(systole-inclusive) achieves fitness $0.009078$ at $\sigma = 0.420$,
a $45\%$ improvement. The Jarlskog invariant vanishes ($J=0$) for the
optimal triple due to a homological class collision
($[\texttt{AbA}]=[\texttt{AAb}]=2$ in $H_1=\mathbb{Z}/5$),
consistent with the experimental suppression $|J|\sim3\times10^{-5}$.
The flat $\mathrm{U}(1)$ holonomy predicts $\delta_{\mathrm{CKM}}=72^\circ$
(PDG: $68^\circ$, $4^\circ$ error). The three
