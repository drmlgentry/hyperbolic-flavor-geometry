...
\large (Version 2: Corrected Bifurcation and Cover-Prime Results)}
...
\textbf{Field obstruction} (Corollary~\ref{cor:obstruction}).
Since $\ITF(M)=\Q(\sqrt{-3})$
and $\Q(\sqrt{-3})\cap\Q(\sqrt5)=\Q$, \emph{no} $\gamma\in\pi_1(M)$
can have real eigenvalues $\ph^{\pm k}$ for odd $k\geq1$. The
``odd rungs'' of the golden-ratio length ladder
($\ell=2k\logph$, $k$ odd) are arithmetically forbidden.
...
Let $M$ be a hyperbolic 3-manifold with $\ITF(M)=\Q(\sqrt{-3})$ (as
holds for $M_{\mathrm{PMNS}}$ and $M_{\mathrm{CKM}}$
\cite{maclachlan-reid}). Then no $\gamma\in\pi_1(M)$ satisfies the
hypothesis of Theorem~\ref{thm:bridge} for odd $k\geq1$.
\end{corollary}

\begin{proof}
Traces of elements of $\rho(\pi_1(M))$ lie in $\ITF(M)$. If $\gamma$
satisfied the hypothesis for odd $k$, Theorem~\ref{thm:bridge} gives
...
$\pm\sqrt5F_k\notin\Q(\sqrt{-3})=\ITF(M)$, a contradiction.
\end{proof}

\begin{remark}[What this resolves, and what it does not]
\label{rem:resolution}
This is the corrected answer to the $\Q(\sqrt5)$-in-$\Q(\sqrt{-3})$
question posed in the Introduction: $\sqrt5$ does \emph{not} enter
the trace field at all. The even-$k$ rungs ($\ell=2k\logph$, $k$
even) would require only $L_k\in\Z\subset\Q(\sqrt{-3})$ ---
no obstruction, but also no existence claim; whether such $\gamma$
actually occur in $\pi_1(M)$ is a separate, open, empirical question.
The odd-$k$ rungs are provably absent. The corollary is therefore a
\emph{selection rule on a discrete length spectrum}, falsifiable in
principle: any $\gamma\in\pi_1(M_{\mathrm{PMNS}})$ found with real
eigenvalues $\ph^{\pm k}$ for odd $k$ would refute either this
corollary or the premise $\ITF(M_{\mathrm{PMNS}})=\Q(\sqrt{-3})$.
\end{remark}
...
=\Q$ also, by the same argument as Corollary~\ref{cor:obstruction}.
So $\Lcal(x)\notin\ITF(M)$ for irrational $x$ as well, \emph{unless}
$\Lcal(x)\in\Q$ (which does not happen for $x\in(0,1)$).
Consequently the previously-noted match
$\sigma_{\mathrm{opt}}\approx\tfrac32\logph$ (i.e.\ $x=3/4$, agreement
$\approx0.004\%$) should be understood as a \textbf{numerical length
coincidence}, not an exact statement that
$|\tr(\gamma_2)|=\Lcal(3/4)$ exactly. We use $\Lcal$ (not $L$) for
...
\begin{proof}
See the companion paper~\cite{gentry-z5bridge}, Theorems 2.6, 5.2,
...
versus $0\%$ for $M_{\mathrm{PMNS}}$ and $M_{\mathrm{CKM}}$ at the
single cover each possesses. With only one or zero covers per
...
(Corollary~\ref{cor:obstruction}), arising from a clean field-theoretic
...
\paragraph{Open directions.} (1) Does $\pi_1(M_{\mathrm{PMNS}})$
contain \emph{any} element with real eigenvalues $\ph^{\pm k}$ for
...
and identification of the bifurcation error corrected in this
version. All mathematical results were conceived, proved, and
