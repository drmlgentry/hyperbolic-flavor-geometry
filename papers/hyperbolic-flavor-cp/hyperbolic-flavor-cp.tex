...
$\delta_{\mathrm{CP}} = \phi_{aa} - \phi_{ab} + \phi_{aB} = 203.5°$,
within $3.3\%$ of the PDG 2024 value of $197°$ with no free parameters.
...
$e^{t(\gamma)}$, while the full complex eigenvalue
$\lambda(\gamma)$ carries both $t$ and $\phi$.

The main results are:
\begin{enumerate}
\item \textbf{CP phase prediction} (Section~\ref{sec:prediction}):
The combination $\phi_{aa} - \phi_{ab} + \phi_{aB} = 203.5°$
...
Word & $t$ & $\phi$ (deg) & $e^{|t|}$ \\
\midrule
\texttt{aa} & $-0.8894$ & $-168.6°$ & $2.4338$ \\
\texttt{ab} & $-0.4992$ & $+83.7°$  & $1.6473$ \\
\texttt{aB} & $-0.4447$ & $+95.7°$  & $1.5601$ \\
\bottomrule
\end{tabular}
\end{table}
...
Numerically:
\begin{align}
\delta_{\mathrm{geom}} &= (-168.6°) - (83.7°) + (95.7°) \nonumber \\
&= -156.6° \equiv 203.5° \pmod{360°}.
\label{eq:delta_num}
\end{align}
The PDG 2024 central value is $\delta_{\mathrm{CP}} = 197°$,
giving a discrepancy of $6.5°$ or $3.3\%$.
...
$\phi_{aa}/2 = \phi_a = -84.3°$ is the fundamental twist of
...
\end{equation}
where $\alpha \in [0,1]$ interpolates between the real ($\alpha=0$)
...
Word & $t$ & $\phi$ (deg) & $e^{|t|}$ \\
\midrule
\texttt{aaB} & $+0.8859$ & $92.49°$ & $2.425$ \\
\texttt{AbA} & $+0.8859$ & $92.49°$ & $2.425$ \\
\texttt{AAb} & $+0.8859$ & $92.49°$ & $2.425$ \\
\bottomrule
\end{tabular}
\end{table}
...
$t = 0.8859$ and $\phi = 92.49° \approx \pi/2$.
...
The common twist angle $\phi \approx 92.49° \approx \pi/2$
is notable: $e^{i\phi} \approx i$, so the holonomy eigenvalue
is nearly purely imaginary. We observe that
$7\theta_{12}^{\mathrm{CKM}} = 7 \times 13.04° = 91.28°$,
within $1.2°$ of $\phi$, suggesting a possible connection
...
The translation lengths $e^{|t|}$ of the three optimal PMNS words
...
$\delta_{\mathrm{CP}} = 203.5°$, within $3.3\%$ of the PDG value
of $197°$, with no free parameters.
...
All three words are isospectral with $\phi \approx 92.5°$.}
