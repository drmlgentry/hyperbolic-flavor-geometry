...
\usepackage{amsmath,amssymb,graphicx,hyperref,xcolor,booktabs}
...
The fitness metric and its statistical significance were cross-validated using the \texttt{LatticeFit} package~\cite{Gentry:LatticeFit}.
\end{itemize}
...
structure in $L$ â€” not reducible to row phases.
...
These $\ell_{ij}$ are genuinely complex â€” not reducible to
row phases â€” and generate $J \neq 0$.
...
at fitness $0.049$ and $J = -0.017$ â€” an improvement over the
...
$J = J_{\mathrm{PDG}}$ requires length-3 holonomy words â€” a
...
\bibliography{gentry-hyperbolic-flavor-cpNotes}
