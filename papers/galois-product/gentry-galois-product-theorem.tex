...
\subjclass[2020]{Primary 11R32; Secondary 11R04, 57K32}
...
criteria.
The four fields have discriminants $-3$, $-283$, $-7$, and $-59$; their
Galois closures have groups $C_2$, $S_4$, $C_2$, and $S_3$ respectively,
where the quadratic fields are already Galois. We prove that
...
K_4 &= \Q(t),\quad t^3+2t+1=0, & \disc(K_4)&=-59.
\end{align*}
Their Galois closures have groups $C_2$, $S_4$, $C_2$, and $S_3$ over
$\Q$ respectively; $K_1$ and $K_3$ are already Galois, while $K_2$ and
$K_4$ are not.
...
\textbf{$K_4=\Q(t)$, $t^3+2t+1=0$.} An irreducible cubic with
discriminant $-59$ (see~\S4.1 for the elementary Galois-group argument).
The original cusp-shape coordinate $\beta$ satisfies
$8\beta^3-24\beta^2+28\beta-11=0$, related to $t$ by $t=2\beta-2$
(equivalently $\beta=(t+2)/2$); we use $t$ throughout the remainder of
the paper as the more economical generator. Its Galois closure $L_4$
satisfies $[L_4:\Q]=6$ and $\Gal(L_4/\Q)\cong S_3$.

\begin{proposition}[Trace field identification]\label{prop:manifolds}
Using SnapPy's exact algebraic field-recognition routine, the invariant
trace fields of the cusped census manifolds $m003$, $m019$,
$m010$, $m006$ are, respectively, $K_1$, $K_2$, $K_3$, $K_4$:
\begin{align*}
k(m003)&\cong K_1, & k(m019)&\cong K_2,\\
k(m010)&\cong K_3, & k(m006)&\cong K_4.
\end{align*}
(For $m006$ the recognized defining polynomial is $x^3+2x-1$; the
substitution $x=-t$ gives $t^3+2t+1=0$, confirming this is $K_4$ with
matching discriminant $-59$.)
\end{proposition}

\begin{proposition}[Cusp-Order Distinction for $K_3$]\label{prop:cusporder}
The manifolds $m009$ and $m010$ both have invariant trace field
$\Q(\sqrt{-7})$ and share volume, tetrahedron count, and symmetry group,
but are not isometric (confirmed directly in SnapPy; their length spectra
differ). Their cusp shapes are
\[
\tau_{m009}=\sqrt{-7},\qquad \tau_{m010}=\frac{1+\sqrt{-7}}{2},
\]
confirmed exactly---not merely numerically---by recognizing each as a
root of its minimal polynomial over $\Q$ and verifying the algebraic
identity $\tau_{m009}=2\tau_{m010}-1$ in $K=\Q(\sqrt{-7})$. Hence the
cusp lattice of $m009$ is generated by the nonmaximal order
$\Z[\sqrt{-7}]$, while that of $m010$ is generated by the maximal order
$\mathcal{O}_K$, of index $2$.

Both manifolds are cusped, so their invariant quaternion algebra is
automatically split, $A(\Gamma)\cong M_2(K)$; since $K$ has no real
places, the real-place ramification condition in the arithmetic
Kleinian-group criterion is vacuously satisfied. What remains is trace
integrality of $\Gamma^{(2)}$---the subgroup generated by squares, whose
trace field is $K$ by the Neumann--Reid construction~\cite{NR}. We
checked this on the four Reidemeister--Schreier generators of
$\Gamma^{(2)}$, all $2^4-1=15$ ordered subset-product traces among them,
and all $64$ triple products, for both manifolds: every trace checked
lands exactly in $\mathcal{O}_K$, with no exceptions. This is extensive
computational evidence, not a formal closed-form certificate that every
element of the infinite group $\Gamma^{(2)}$ has integral trace, and we
state it as such: it strongly supports arithmeticity of both $m009$ and
$m010$, and hence---since both are cusped with the same invariant trace
field---their commensurability. Within this commensurability class,
$m010$ is distinguished at the tied minimal volume by realizing the
maximal cusp order.
\end{proposition}
...
\emph{$\Gal(L_4/\Q)\cong S_3$.} The defining polynomial $t^3+2t+1$ is
irreducible modulo $3$ and has discriminant $-59$, a nonsquare. An
irreducible cubic with nonsquare discriminant has Galois
...
\begin{proof}[Proof via degree multiplication]
...
Minkowski's theorem. The exponent $e$ marked in each prime box is
$v_p(\disc)$ for that piece; feeding these into the tower formula of
Corollary~\ref{cor:disc} gives the uniform value $288$ at all four
primes in $L$.}
...
$K_4$ pointwise is a point stabilizer on the three roots of $t^3+2t+1$,
hence
...
\section{Artin Representations and the Conductor-Discriminant
Formula}\label{sec:artin}

\begin{proposition}\label{prop:artin-count}
Every irreducible complex representation of
$G=\Gal(L/\Q)\cong C_2\times S_4\times C_2\times S_3$ is an external
tensor product $\rho=\rho_1\boxtimes\rho_2\boxtimes\rho_3\boxtimes\rho_4$
of irreducibles of the four factors. Consequently $G$ has exactly
$2\cdot5\cdot2\cdot3=60$ irreducible representations, with dimension
distribution
\[
\begin{array}{c|ccccc}
\dim\rho & 1 & 2 & 3 & 4 & 6\\\hline
\text{count} & 16 & 16 & 16 & 4 & 8
\end{array}
\]
and $\sum_{\rho}(\dim\rho)^2=576=|G|$.
\end{proposition}
\begin{proof}
Standard representation theory of direct products. Directly verified in
SageMath by constructing $G$ as an explicit permutation group of degree
$11$ and computing its full character table.
\end{proof}

\begin{proposition}[Conductor-discriminant check]\label{prop:artin-conductor}
For each ramified prime $p\in\{3,7,59,283\}$, inertia in $G$ has order
$2$: trivial in three factors, and generating the remaining one. At
$p=283$: since $v_{283}(\disc K_2)=1$, ramification is tame with a
single collision, so inertia acts on the roots of $x^4-x-1$ as a
transposition, not a double transposition. Since $2\nmid p$ for every
$p\in\{3,7,59,283\}$, all four inertia groups act tamely, so the tame
conductor formula applies uniformly. For an order-$2$ generator
$\sigma$, the tame Artin conductor of $\rho$ at $p$ is
\[
a_p(\rho)=\dim\rho-\dim\rho^{\langle\sigma\rangle}
=\frac{\dim\rho-\chi_\rho(\sigma)}{2}.
\]
Summing over all $60$ irreducibles,
\[
\sum_{\rho\in\widehat G}\dim(\rho)\,a_p(\rho)=288
\qquad\text{for each }p\in\{3,7,59,283\}.
\]
\end{proposition}
\begin{proof}
At a transposition, the tame conductor exponent
$a=(\dim\rho-\chi_\rho(\sigma))/2$ takes the following values on the
irreducibles of $S_4$ and $S_3$, independently confirmed against Sage's
character tables:
\[
\begin{array}{c|ccccc}
S_4 & [4] & [1^4] & [2,2] & [3,1] & [2,1,1]\\\hline
\dim & 1 & 1 & 2 & 3 & 3\\
a & 0 & 1 & 1 & 1 & 2
\end{array}
\qquad
...
\end{array}
\]
Building $G$ directly as a
permutation group of order $576$, identifying the four inertia
generators from its natural block structure, and summing
$\dim(\rho)\,a_p(\rho)$ over all $60$ irreducible complex characters
(which are real-valued in this case) gives $288$ for each of
$p=3,7,59,283$.
\end{proof}

By the Artin conductor--discriminant formula,
$v_p(\disc L)=\sum_\rho\dim(\rho)\,a_p(\rho)$, so
Proposition~\ref{prop:artin-conductor} gives a second, independent proof
of Corollary~\ref{cor:disc}: the uniform ramification exponent $288$,
and hence $\disc(L)=350637^{288}$, obtained here from the representation
theory of $G$ rather than from the tower discriminant formula.
...
manifold is proposed in that program; the arithmetic independence of the
$K_1$/$K_2$ ($m003$/$m019$) pair specifically is established
in~\cite{DualSurgery}. Theorem~\ref{thm:main} and
...
\item \textit{Canonicity of $m010$.} By Proposition~\ref{prop:manifolds},
  ...
  and $m006$. It is \emph{not} the unique minimal-volume census manifold
  with this trace field: Proposition~\ref{prop:cusporder} shows $m010$ is
  tied at minimal volume with the non-isometric manifold $m009$, and is
  instead distinguished among the two by realizing the maximal cusp order
  $\mathcal{O}_K$ rather than the index-$2$ suborder $\Z[\sqrt{-7}]$. What
  remains open is whether this maximal-order property is itself a genuine
  canonical selector---in particular, whether $m010$ is the unique
  minimal-volume census manifold with this trace field \emph{and} a
  maximal cusp order, and whether that property has a clean
  characterization within the arithmetic commensurability class of $K$.
\item \textit{Covering tower structure.} Related Lucas/Lehmer behavior in
  hyperbolic 3-manifold covering-tower homology is known~\cite{MR97};
  whether the specific covering towers of $m003$ and $m006$ fit into
  this framework, and whether an analogous structure extends to the
...
\item \textit{Geometric mechanism.} The theorem establishes an algebraic
  ...
  Dimofte-Gaiotto-Gukov~\cite{DGG} provides a geometric mechanism
  elevating this to a dynamical statement about gauge interactions
  remains an open problem.
\item \textit{Rarity of the configuration.} The proof method here---four
  Galois pieces with pairwise disjoint ramification support---does not
  by itself imply that this specific configuration is arithmetically
  distinguished: for census trace fields with large, essentially random
  ramified primes, disjoint support is common rather than exceptional,
  since large primes rarely collide. This question is now answered in
  full by a census-wide scan; see
  \S\ref{sub:census-rarity} below.
\end{enumerate}

\subsection{Census rarity of the distinguished ramification
tuple}\label{sub:census-rarity}

As a computational control, we scanned all $212{,}641$ manifolds in the
SnapPy \texttt{OrientableCuspedCensus} under fixed trace-field
recognition and Galois-closure bounds.\footnote{Script
\texttt{census\_disjoint\_ramification\_scan.sage.py}
in~\cite{HFGcode}; final checkpoint SHA-256
\texttt{f6d76c553f6e4564a053aeb42ae5cc7062d5912ac875e91d2f4fc229a40a5c40}.}
The pipeline successfully resolved $1{,}839$ manifolds, representing
$165$ distinct number fields.

Among these $165$ resolved fields, there are
\[
4{,}296{,}004
\]
four-tuples of distinct fields having pairwise-disjoint ramification
support. Of these,
\[
3{,}648
\]
have Galois-group multiset $\{C_2,S_4,C_2,S_3\}$, and therefore realize
the same direct-product mechanism as Theorem~\ref{thm:main}. Thus the
arithmetic mechanism has substantial multiplicity within the resolved
field set, rather than being peculiar to a single four-field
construction.

The particular ramification data of the present example are
substantially more restrictive. Among the resolved fields, singleton
ramification support $\{3\}$, $\{283\}$, and $\{59\}$ occurs for
exactly one field each, while support $\{7\}$ occurs for two fields.
Among all $3{,}648$ disjoint-support four-tuples of type
$\{C_2,S_4,C_2,S_3\}$, exactly one has ramification-prime set
\[
\boxed{\{3,7,59,283\}}.
\]
It is precisely the tuple $K_1,K_2,K_3,K_4$ used in
Theorem~\ref{thm:main}. Hence, within the subset resolved by this
computation, the present construction is the unique realization of its
Galois-product type carrying this exact four-prime ramification
signature. This uniqueness also admits a direct structural
explanation: after restricting to the $86$ resolved fields of the
required Galois types $C_2$, $S_4$, and $S_3$, each required singleton
ramification support $\{3\}$, $\{7\}$, $\{59\}$, and $\{283\}$ is
represented by exactly one field of the corresponding required type,
so the distinguished four-field tuple is forced. The count $3{,}648$
itself is not obtained by treating the four field choices
independently: exact enumeration over the $13$ ramification-disjoint
$C_2$-pairs shows that admissible $S_4$ and $S_3$ counts vary with the
accumulated ramification support; the accompanying reproducibility
computation evaluates the resulting finite compatibility sum exactly.

This statement should not be interpreted as a frequency result for the
complete census. Only $1{,}839/212{,}641=0.865\%$ of census manifolds
were successfully resolved under the fixed computational bounds, and
recognition rates are strongly family-dependent, ranging from $30.6\%$
in the \texttt{m} census family to $0.62\%$ in the \texttt{o} family.
The resolved fields therefore constitute a computationally selected
subset rather than a random sample of census trace fields; uniqueness
within this subset should be read as \emph{conditional uniqueness under
a precisely stated finite search}, not as a claim of absence of
selection bias.

In summary, three logically distinct claims are established here:
(i) the algebraic mechanism of Theorem~\ref{thm:main}---disjoint
ramification implies a direct product of Galois groups---is general
and unconditional; (ii) among the $165$ number fields resolved by the
census scan, $3{,}648$ four-tuples realize the same
$C_2\times S_4\times C_2\times S_3$ type; (iii) among those $3{,}648$,
exactly one carries the specific ramification-prime set
$\{3,7,59,283\}$ used above.

As a methodological remark, the count of distinct resolved fields grew
non-uniformly over the course of the scan: $102$ fields at $25.4\%$ of
the census, $129$ at $72.5\%$, $132$ at $86.2\%$, and $165$ at
completion. In particular $33$ new fields appeared in the final
$13.8\%$ of the scan, versus only $3$ in the comparably sized preceding
interval ($72.5\%$ to $86.2\%$)---indicating the resolved field
population had not saturated at the earlier checkpoints, and that a
full-census run was necessary for the multiplicity count above to be
reliable.
...
of rational primes splitting completely in $L$ is $1/576$, and more
generally that the joint Frobenius class $(c_1,c_2,c_3,c_4)$ (conjugacy
classes in the four factors) occurs with density
$|c_1||c_2||c_3||c_4|/576$.

Testing all $3241$ unramified primes below $30000$, using the monic
model $t^3+2t+1$ throughout (the non-monic geometric model
$8\beta^3-24\beta^2+28\beta-11$ would give a degenerate reduction at
$p=2$, since its leading coefficient $8$ vanishes mod $2$): all $60$
possible joint Frobenius classes occur, and the four defining
polynomials split completely at exactly $5$ primes,
against an expected count of $3241/576\approx5.63$ under the theoretical
density---consistent with, though of course not an independent proof
of, Theorem~\ref{thm:main}.

The marginal splitting-type distributions for the two non-abelian
factors are, similarly, an empirical illustration of the already-proved
direct-product Chebotarev law, not an independent statistical proof of
it:
\[
\begin{array}{c|ccccc}
S_4\text{ type} & 1^4 & 2,1,1 & 2,2 & 3,1 & 4\\\hline
\text{observed} & 119 & 830 & 409 & 1062 & 821\\
\text{expected} & 135.04 & 810.25 & 405.13 & 1080.33 & 810.25
\end{array}
\]
\[
\begin{array}{c|ccc}
S_3\text{ type} & 1^3 & 2,1 & 3\\\hline
\text{observed} & 531 & 1618 & 1092\\
\text{expected} & 540.17 & 1620.50 & 1080.33
\end{array}
\]
with expected counts given by (conjugacy class size)$\times3241/|G_i|$.
...
\url{https://github.com/drmlgentry/hyperbolic-flavor-geometry},
commit \texttt{fd8ffa5} or later (2026).
\bibitem{DGG}
T.~Dimofte, D.~Gaiotto, and S.~Gukov,
Gauge theories labelled by three-manifolds,
\emph{Comm.\ Math.\ Phys.} \textbf{325} (2014), no.~2, 367--419.
\bibitem{DualSurgery}
M.~Gentry,
