...
Both manifolds have invariant trace field $K$, which has exactly one
complex place and no real places, so the invariant quaternion algebra is
automatically split. Trace integrality of $\Gamma^{(2)}$---the subgroup
generated by squares, whose trace field is $K$ by the Neumann--Reid
construction~\cite{NR}---was checked on an extensive finite generating
set for the trace ring (all $2^4-1=15$ ordered subset-product traces of
the four Reidemeister--Schreier generators of $\Gamma^{(2)}$, together
with all $64$ triple products, for both manifolds): every trace checked
lands exactly in $\mathcal{O}_K$, with no exceptions. This strongly
supports arithmeticity of both $m009$ and $m010$, and hence---since both
are cusped with the same invariant trace field---their commensurability.
Within this commensurability class, $m010$ is distinguished at the tied
minimal volume by realizing the maximal cusp order.
\end{proposition}
...
transposition, not a double transposition. For an order-$2$ generator
