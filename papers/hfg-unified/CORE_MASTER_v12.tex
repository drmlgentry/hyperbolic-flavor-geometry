...
is a topological consequence at zeroth order, not a coincidence.
Verified numerically for $10^3$ random same-trace triples ($J=0$ to
machine precision in all cases).
\textbf{Note:} The observed quark Jarlskog invariant
$J_\mathrm{exp}\approx3\times10^{-5}$ requires symmetry-breaking
corrections beyond the current zeroth-order construction.
...
\begin{figure}[htbp]
\centering
% [Figure: fig1_phi_lattice_masses --- file not included in this version]
...
\paragraph{Test of primitive geodesic length encoding.}
A direct test of the hypothesis that primitive geodesic lengths equal
$(q/4)\log\phi$ for the mass lattice indices $q$ shows no enrichment.
For $M_{\mathrm{PMNS}}$, there are $638$ distinct geodesic lengths up
to $\ell=6$; the prime geodesic theorem predicts $\approx52$ random
matches for the $49$ $\phi$-lattice points within tolerance $0.005$.
Only $21$ matches are observed, giving an enrichment factor $0.40$
--- depletion, not enhancement. Therefore the claim that primitive
geodesic lengths directly encode the mass lattice is \emph{not
supported} at the level of raw length counts.
The arithmetic connection between the $\phi$-lattice and the manifolds
lies deeper: in the Lucas--Eisenstein theorem
(Section~\ref{sec:lucas}), the lepton mass ratios
$m_\mu/m_e\approx L_{11}=199$ and $m_\tau/m_e\approx L_{17}=3571$
are Eisenstein norms in the ring of integers of the trace field
$\mathbb{Q}(\sqrt{-3})$ of $M_{\mathrm{PMNS}}$.


\subsection{Neutrino mass prediction (speculative)}
\label{subsec:neutrino_brief}

The $\phi$-lattice is an empirical pattern in the \emph{charged}
fermion masses. Its extension to neutrinos is not supported by
direct geometric evidence from hyperbolic 3-manifolds and must
be regarded as speculative. Subject to this caveat, requiring
neutrino masses to fall on $\phi$-lattice points consistent
with oscillation data (normal ordering) yields a unique triplet:
$m_1\approx8.4$~meV, $m_2\approx12.0$~meV, $m_3\approx50.8$~meV
($\sum m_\nu\approx71$~meV), within the Planck~2018 bound
($<120$~meV) and testable by CMB-S4 ($\sim30$~meV sensitivity).
This prediction will be falsified or supported by future
cosmological data, independently of the HFG mixing and CP results.
...
\begin{equation}
  ...
  = \phi^k + \phi^{-k} = L_k
  \quad\Longleftrightarrow\quad \ell = 2k\log\phi.
...
\paragraph{Lucas--Eisenstein Lepton Mass Theorem.}
The integers $L_{11}=199$ and $L_{17}=3571$ are primes congruent
to $1\pmod{3}$, hence split in $\mathbb{Q}(\sqrt{-3})$, and are
norms in the Eisenstein integers:
\[
  199 = N(-15-13\omega),\qquad 3571 = N(-69-34\omega),
\]
where $N(a+b\omega)=a^2-ab+b^2$ and $\omega=e^{2\pi i/3}$.
They approximate the PDG lepton mass ratios $m_\mu/m_e=206.77$
($3.8\%$ error) and $m_\tau/m_e=3477.2$ ($2.7\%$ error).
Both $199,3571\equiv3\pmod{4}$ and are therefore \emph{not}
Gaussian norms, arithmetically locking the lepton masses to the
trace field $\mathbb{Q}(\sqrt{-3})$ of $M_{\mathrm{PMNS}}$.
The next element $L_{19}=9349$ predicts a particle at
$m_e\cdot\varphi^{19}=4.78$~GeV with no known Standard Model
assignment.

\paragraph{Prime dictionary.}
\textbf{[Corrected in v12.]}
The algebraic covering tower of $M_\mathrm{PMNS}$, computed via
\texttt{M.covers(d)} for degrees $d=2,\ldots,6$, has exactly one cover:
a degree-5 cover with $H_1\cong(\mathbb{Z}/11)^2$~\cite{Gentry:Lucas}.
The only new algebraic cover prime is $11 = L_5$.

Earlier versions of this paper claimed the prime dictionary
$\{2,3,7,11,29\}$; this conflated algebraic covers with census
volume coincidences (manifolds at integer multiples of
$\mathrm{vol}(M_\mathrm{PMNS})$, which are commensurable neighbours,
not covers). Primes $\{2,3,7,29\}$ appear in the commensurability class
but not in algebraic covers. The Farey cover prime formula
$p_n = 5n(n+1)+1$ (verified $n=1,\ldots,10$) gives $p_1=11=L_5$
as the unique Lucas prime in the sequence~\cite{Gentry:Lucas}.
...
For $M_\mathrm{PMNS}$, the unique algebraic cover prime through degree~6
is $11=L_5$~\cite{Gentry:Lucas}.
For $M_\mathrm{CKM}$, \texttt{M.covers(d)} returns no covers through
degree~6 in the closed census.
The Farey chain $M_n=m003(-n,2n+1)$ has degree-5 cover prime
$p_n=5n(n+1)+1$; only $n=1$ (i.e., $M_\mathrm{PMNS}$) gives a Lucas
prime ($p_1=11=L_5$), making it the unique Lucas-prime element of its
canonical Farey tower.
...
\begin{figure}[htbp]
\centering
% [Figure: fig2_null_distribution --- file not included in this version]\hfill
% [Figure: fig3_sector_anchor_null --- file not included in this version]
...
\section{New geometric results (May 2026)}
\label{sec:new_results}

The following three results were established after the initial
version of this paper and significantly strengthen the geometric
foundations of the HFG framework.

\subsection{Surgery formula (theorem)}
...
The fundamental group $\pi_1(m003) = \langle a,b \mid abA^2bab^3\rangle$
abelianises to $H_1(m003) = \mathbb{Z}\alpha\oplus\mathbb{Z}/5\beta$.
The peripheral curves abelianise to $[\mu]=-2\alpha-3\beta$ and
$[\lambda]=-\alpha+\beta$.
Dehn surgery at $(p,q)$ kills $p[\mu]+q[\lambda]$; the surgery
matrix has determinant $5|2p+q|$.
\end{proof}
\begin{corollary}
$H_1(M_\mathrm{PMNS}) = \mathbb{Z}/5$ as a theorem:
slope $(-2,3)$ gives $|2(-2)+3|=1$, so $|H_1|=5$.
\end{corollary}
Verified against 14 independent hyperbolic fillings~\cite{Gentry:Unified}.

\subsection{Eisenstein cusp (theorem)}
\begin{theorem}[Cusp shape of $m003$]
The cusp shape of the cusped ancestor $m003$ is exactly
$\tau = e^{i\pi/3} = \omega$ (the Eisenstein unit), with
residual $|\tau_\mathrm{SnapPy} - e^{i\pi/3}| < 10^{-15}$.
\end{theorem}
The proof follows from the trace field $\mathbb{Q}(\sqrt{-3})$:
for arithmetic hyperbolic 3-manifolds, the cusp shape lies in
the trace field~\cite{MaclachlanReid}, and $e^{i\pi/3}$ is the
canonical generator of $\mathbb{Q}(\sqrt{-3})$.
The cusp translations are $\mu_\mathrm{geom}=1+\sqrt{3}\,i$ and
$\lambda_\mathrm{geom}=2$, giving area $2\sqrt{3}$ (equilateral
Eisenstein torus).
This provides the arithmetic foundation for the Eisenstein norm
encoding of lepton masses (Section~\ref{sec:lucas}).

\subsection{Farey tower cover prime formula (computational theorem)}
\begin{theorem}[Cover prime formula, computational]
The Farey tower $M_n = m003(-n,2n+1)$, $n\geq1$, consists of an
infinite family of closed hyperbolic 3-manifolds, all with
$H_1=\mathbb{Z}/5$, with volumes increasing to
$\mathrm{vol}(m003_\mathrm{cusp}) = 2.0299$.
The unique degree-5 algebraic covering space of $M_n$ has
\[
  H_1(M_n^{(5)}) \cong (\mathbb{Z}/p_n)^2,
  \qquad p_n = 5n(n+1)+1,
\]
verified for $n=1,\ldots,10$~\cite{Gentry:Lucas}.
\end{theorem}
The sequence of primes is $11, 31, 61, 101, 151, 211, 281, \ldots$
(all prime for $n\leq7$, then composite).
$M_1 = M_\mathrm{PMNS}$ is the unique element with a Lucas prime cover
($p_1 = 11 = L_5$), establishing it as the minimum-volume element
of a canonical infinite family.

\subsection{Volume adjacency}
$M_\mathrm{CKM}$ has volume $2.0289$, lying within $0.05\%$ of
$\mathrm{vol}(m003_\mathrm{cusp}) = 2.0299$ — the asymptotic limit
of the PMNS Farey tower.
The two manifolds are not commensurable (distinct invariant trace
fields imply distinct commensurability classes for arithmetic
hyperbolic 3-manifolds~\cite{MaclachlanReid}).
This volume adjacency is unexplained and may reflect a geometric
bridge between the two sectors.
...
\bibitem{Gentry:Twist}
  ...
  in preparation (corrections to CP formula and $M_Z/M_W$ words pending).
...
\bibitem{Gentry:neutrino}
  M.~L.~Gentry,
  ``Neutrino Masses from the Golden Ratio Lattice:
  A Parameter-Free Prediction with Arithmetic Uniqueness,''
  SSRN \href{https://doi.org/10.2139/ssrn.6631218}{6631218} (2026).

\bibitem{Gentry:Covering}
  M.~L.~Gentry,
  ``Arithmetic of Dehn Filling Slopes and the Homology of the Flavor Covering Tower,''
  SSRN 6761981 (2026, removed); results superseded by~\cite{Gentry:Lucas}.
...
\bibitem{Eto2025}
  ...
  ``Stable knot solitons in a realistic Standard Model extension,''
  Phys.\ Rev.\ D (2025), in preparation.
