...
\node[box] (arith)
  {Arithmetic Structure\\
   $\mathbb{Q}(\sqrt5)$ unit lattice, $H_1=\mathbb{Z}/5$};
\node[box, below=of arith] (sym)
  {Symmetric Space\\
   $\mathrm{SL}(2,\mathbb{R})/\mathrm{SO}(2)$};
\node[box, below=of sym] (mass)
  {Mass Hierarchy\\
   $m \approx m_e\,\varphi^{q/4}$};
\node[box, below left=1.8cm and 2.6cm of mass] (ckm)
  {CKM Mixing\\($K$-factor)};
\node[box, below=1.8cm of mass] (cp)
  {CP Violation\\($A$-factor)};
\node[box, below right=1.8cm and 2.6cm of mass] (pmns)
  {PMNS Mixing\\($N$-factor)};
\draw[arrow] (arith) -- (sym)
  node[midway, right, font=\scriptsize] {exp map};
\draw[arrow] (sym) -- (mass)
  node[midway, right, font=\scriptsize] {lattice points};
\draw[arrow] (mass) -- (ckm);
\draw[arrow] (mass) -- (cp);
\draw[arrow] (mass) -- (pmns);
\node[above=0.5cm of arith, font=\scriptsize, align=center]
  {$\mathbb{Z}/5$ torsion $\to$ $\zeta_5$ $\to$ $\mathbb{Q}(\sqrt5)$ $\to$ $\log\varphi$};
\end{tikzpicture}
\caption{Conceptual map of the unified geometric flavor framework.
The arithmetic structure ($\mathbb{Q}(\sqrt5)$ unit lattice and
$H_1=\mathbb{Z}/5$ torsion) generates both the mass hierarchy via
the symmetric space exponential map and the mixing/CP sector via
...
\node[topbox] (arith) at (0,4) {
  Arithmetic structure:\\
  $\mathbb{Q}(\sqrt{5})$ unit lattice, $H_1(M)=\mathbb{Z}/5$
};
...
\node[bluebox, below=of arith, yshift=-1.5cm] (cp) {
  CP parity obstruction theorem\\[2pt]
...
$\mathrm{PSL}(2,\mathbb{C})=KAN$. The chirality--CP obstruction theorem
explains the quark--lepton CP asymmetry geometrically.}
\label{fig:framework}
\end{figure}
\begin{figure}[htbp]
\centering
...
\node[topbox] (arith) at (0,4) {
  Arithmetic structure\\
  $\mathbb{Q}(\sqrt{5})$, $H_1(M)=\mathbb{Z}/5$
};
\node[bluebox] (symm) at (-3.5,2) {
  Symmetric space\\
  $\mathrm{SL}(2,\mathbb{R})/\mathrm{SO}(2)$
};
\node[bluebox] (hyp) at (3.5,2) {
  Hyperbolic 3-manifolds\\
  $m003,\; m006$
};
\node[greenbox] (mass) at (-3.5,0) {
  Mass hierarchy\\
  $m \sim m_e\,\phi^{q/4}$
};
\node[bluebox] (cp) at (0,0) {
  CP obstruction theorem\\[2pt]
  Amphicheiral $\Leftrightarrow$ $g_*=-\mathrm{id}$\\
  Chiral $\Leftrightarrow$ no such $g$
};
\node[greenbox] (iwasawa) at (3.5,0) {
  Iwasawa decomposition\\
  $\mathrm{PSL}(2,\mathbb{C})=KAN$
};
\node[redbox] (ckm) at (-3.5,-2) {
  CKM mixing\\
  ($m006$, $K$-factor)
};
\node[redbox] (phase) at (0,-2) {
  CP phase\\
  $e^{2\pi ik/5}$
};
\node[redbox] (pmns) at (3.5,-2) {
  PMNS mixing\\
  ($m003$, $N$-factor)
};
\draw[->] (arith) -- (symm);
...
\end{tikzpicture}
\caption{Unified geometric flavor framework. The arithmetic structure
generates both the mass hierarchy and the mixing/CP sector.
The chirality--CP theorem (centre) explains $J=0$ for amphicheiral
$m006$ and $J\neq0$ for chiral $m003$.}
\label{fig:framework}
\end{figure}
...
A census scan establishes: census index~39 is a genuine degree-2 covering
space of $m003$, with $\mathrm{vol}(\mathrm{idx}_{39})/\mathrm{vol}(m003)
= 2.0000000000$ (exact to 10 s.f.) and $H_1 = \mathbb{Z}/55 \cong
\mathbb{Z}/5\times\mathbb{Z}/11$, confirmed by ten shared geodesics.
The projection $\mathbb{Z}/55\twoheadrightarrow\mathbb{Z}/5$ preserves the
$\mathbb{Z}/5$ torsion. By contrast, $m006$ has no degree-2 cover
preserving $\mathbb{Z}/5$ (closest candidate: index~1275, $H_1=\mathbb{Z}/3$,
zero shared geodesics). The structural asymmetry---$m003$ decomposable,
$m006$ irreducible---mirrors the phenomenological lepton--quark mixing
asymmetry. This is a theorem, not a fit.
...
volume (index~1275, $H_1 = \mathbb{Z}/3$) shares zero geodesics
with $m006$. This asymmetry --- $m003$ is geometrically
