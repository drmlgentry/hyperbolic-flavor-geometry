...
All three CKM words $\{\mathrm{aaB},\mathrm{AbA},\mathrm{AAb}\}$ on $m006$
have identical trace $\mathrm{Tr}(\rho(w))=-0.1231+2.0108i$,
placing them in the same conjugacy class of $\mathrm{SU}(2)$.
...
\[
  \sigma_\mathrm{opt} = \ell_2(M_\mathrm{CKM}) = 0.4906
  = \tfrac{3}{2}\log\phi \pm 0.004\%,
\]
with $|\mathrm{tr}| = L_{1/2} = \phi^{1/2}+\phi^{-1/2}$, a quarter-Lucas
algebraic number. The smearing parameter is not free---it is determined
by the quarter-Lucas structure of the CKM manifold's geodesic spectrum.
...
A scan of 64 Dehn fillings of $m003$ and 34 fillings of $m006$
reveals a sharp phase transition at $n \approx 18$: for $n \geq 18$,
$\log n$ appears as a geodesic length in more than 80\% of all fillings
(universal tier), while for $n < 18$ geodesic hits are sparse and
filling-dependent (slope-encoded tier). The universal geodesics
correspond precisely to Lucas numbers: $n = 18 \approx L_6$,
$n = 29 \approx L_7$ ($\phi$-residual 0.003), $n = 37 \approx L_{15/2}$
(residual 0.004), $n = 47 \approx L_8$ (residual 0.001), all appearing
in $\geq 84\%$ of fillings. In the sparse region ($n < 18$), fillings
$m003(p,q)$ with $|p| \leq 5$ are 3--7$\times$ enriched for geodesics
at $\log|p|$ compared to generic fillings---the slope arithmetic is
encoded in the sparse-region geodesics. The two-tier structure reflects
the Margulis thick-thin decomposition: the universal tier corresponds to
geodesic lengths above the Margulis constant, while the sparse tier
carries genuine arithmetic information about the Dehn filling slope.
...
The Weeks manifold ($\mathrm{OrientableClosedCensus}[0]$) is the unique
closed orientable hyperbolic 3-manifold of smallest volume.
Its key invariants, computed via \texttt{Sage+SnapPy}, are:
volume $= 0.94270736$, $H_1 = \mathbb{Z}/5 \oplus \mathbb{Z}/5$,
and isometry group $D_6$ (dihedral group of order~12).
...
the same cusped ancestor that yields the PMNS manifold via the $(-2,3)$
filling (verified: $\mathrm{vol}(m003(-2,3)) = 0.98136883$).
Both fillings are Dehn surgeries on the same arithmetic cusped manifold
with $\mathrm{CS} = 1/4$; the Dedekind sum $s(2,1) = 0$ leaves the
Chern--Simons invariant unchanged.
...
\underbrace{M_\mathrm{CKM}}_{\mathrm{vol}\;=\;2.0289}
\]
with $M_\mathrm{CKM} = \mathrm{OrientableClosedCensus}[43]$ the
non-arithmetic manifold identified with the quark sector.
All three carry $\mathbb{Z}/5$ torsion; the Weeks manifold carries
two independent $\mathbb{Z}/5$ cycles.

The $(-2,3)$ Dehn surgery that produces the PMNS manifold from the
cusped ancestor kills one $\mathbb{Z}/5$ generator, reducing
$H_1(\mathrm{Weeks}) = \mathbb{Z}/5 \oplus \mathbb{Z}/5$ to
$H_1(M_\mathrm{PMNS}) = \mathbb{Z}/5$.
Simultaneously, the isometry group is reduced from $D_6$ (order~12)
to $\mathbb{Z}/2 \times \mathbb{Z}/2$ (order~4), and the geodesic
multiplicities in the length spectrum drop from uniformly~3
(Weeks, reflecting $D_6$ symmetry) to~1 and~2 (PMNS).

\begin{table}[h]
\centering
\caption{Length spectrum comparison: first five geodesics.
Torsion = $\mathrm{Im}(\ell)$ in radians; mult = geodesic multiplicity.}
\label{tab:length_spectra}
\begin{tabular}{lrrrl}
\toprule
Manifold & $\mathrm{Re}(\ell_1)$ & $\mathrm{Im}(\ell_1)$ & mult & Note \\
\midrule
...
The Weeks manifold, with its enhanced symmetry and rank-2 torsion
$\mathbb{Z}/5 \oplus \mathbb{Z}/5$, may represent a more fundamental
geometric phase from which the lepton sector emerges by a topological
reduction. The $\mathbb{Z}/5$ factor that is killed by the
$(-2,3)$ surgery corresponds to one CP-phase degree of freedom; its
removal is consistent with the empirical observation that the PMNS
sector exhibits a single physical CP phase $\delta_\mathrm{CP}$,
while a rank-2 $\mathbb{Z}/5$ structure would support two independent
CP phases.

This Dehn surgery relation establishes a direct topological link
between the smallest closed hyperbolic 3-manifold and the PMNS sector,
reinforcing the special role of $\mathbb{Z}/5$ torsion in the flavor
framework and suggesting that the Weeks manifold belongs to a more
complete theory of which the present framework is the low-energy limit.
...
Whether the arithmetic hyperbolic structure identified here reflects a deeper dynamical principle remains an open question. Immediate directions include the Chern--Simons invariants and Dirac spectrum of these manifolds, the spectral action approach to flavor dynamics, extending the covering tower analysis to other primes $p$, and proving the spectral universality threshold analytically from the Margulis lemma. The neutrino mass lattice~\cite{Gentry:neutrino} provides an independent test of the $\phi^{q/4}$ structure in the lepton sector.
...
M.~L.~Gentry,
\textit{Neutrino Masses from the Golden Ratio Lattice: A Parameter-Free Prediction with Arithmetic Uniqueness},
