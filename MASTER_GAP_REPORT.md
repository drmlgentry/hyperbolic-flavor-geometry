...
**Stage 3A (open question, do NOT attempt yet, no mass indices consulted):**
Stage 1 gives a canonical h in H=C2 (Frobenius, once K subset L is fixed). Stage 2
gives a canonical coset gH in G/H (3 states). Together: 2x3=6 geometric states. But
G/H is a coset SET, not a quotient GROUP -- H (order 2, generated by one specific
transposition) is NOT normal in S3 (none of S3's three order-2 subgroups are). So
there is no automatic group structure on C2 x G/H matching S3 itself. The open
question: does the PAIR (h, gH) canonically reconstruct a specific element g in S3 --
i.e. is there a natural, geometry-only map (h, gH) -> S3 that recovers the full group
element, not just two separate partial data? This must be answered, with no mass
indices involved, before any attempt to connect the six geometric states to the six
quark mass indices (12, 18, 43, 65, 75, 106) -- that mapping is Stage 3, untouched.
