...
2. **Bianchi subgroup identification for m009/m010 — candidate target
   group identified, generator matrices still needed.** `Γ_{009},
   Γ_{010}` are proved not to be subgroups of ordinary T_7=PSL₂(O_{−7})
   (COMPLETED item 34). **Candidate extension, checked this session:**
   B_7=PGL₂(O_{−7}), realized inside PSL₂(C) via determinant-rescaling,
   generated by T_7 plus one projective involution J:z↦−z (J²=1).
   Independently confirmed directly in Sage (not from any relay):
   class_number(Q(√−7))=1, unit group of O_{−7} is C₂ (order 2) — these
   two facts give [PGL₂(O_K):PSL₂(O_K)]=2 by elementary algebra alone,
   hence covol(B_7)=covol(T_7)/2=0.444457463908..., and
   2.66674478344906/0.444457463908...=6.000000... exactly, so IF
   m009/m010 embed in B_7 the index-6 prediction is now internally
   consistent. **What is NOT independently confirmed:** that PGL₂(O_{−7})
   is genuinely *maximal* among discrete extensions (i.e. that it equals
   Vulakh's B_d), rather than merely *a* valid discrete extension — the
   elementary index-2 argument only establishes the latter. Citation
   checked and real (Cambridge's own listing: correct title/author/
   journal/year): L. Ya. Vulakh, "Reflections in extended Bianchi
   groups," *Math. Proc. Cambridge Philos. Soc.* 115(1), 1994, DOI
   10.1017/S0305004100071887 — but the fetched abstract describes
   reflection/Coxeter subgroups and Vinberg's class-number-divides-4
   criterion, not a verbatim maximality statement; the paper's full text
   has not been read, so treat the B_7=PGL₂(O_{−7}) identification as
   plausible and arithmetically consistent, not proved from source.
   **Blocking:** no explicit 2×2 matrix generators or relators for
   either T_7 or B_7 have been sourced. Two checked-real candidate
   sources for T_7 (both correctly attributed to Tanner Reese —
   "Dinakaran (2022)" confirmed a false attribution, no such paper
   exists): Grunewald-Schwermer, *Trans. Amer. Math. Soc.* 335(1), 1993,
   pp. 47–78; and Reese, "Presentations for Singly-Cusped Bianchi
   Groups" — both the arXiv preprint (arXiv:2206.01262, 2022) and the
   published version (*Topology Appl.* 328, 2023, article 108443, DOI
   10.1016/j.topol.2023.108443, confirmed via Crossref metadata). Reese
   covers PSL₂(O_d) for d=−7 explicitly per its own abstract, but is
   about the ordinary group only — not a ready-made B_7 source. Neither
   paper's full text has been read (no institutional access; this
   project will not use piracy routes to obtain it). GAP script
   `reproduce/bianchi_d7_lowindex.g` has a Stage 2 skeleton with explicit
   TODO markers and a hard `QUIT_GAP(1)` if run without real relators —
   do not fill in generators/relators/conjugation words by guessing.
   Once sourced: enumerate B_7's index-6 subgroups, filter torsion-free
   + one-cusp, match against m009's H₁=Z⊕Z/2, Λ=Z[√−7] vs. m010's
   H₁=Z⊕Z/6, Λ=O_K — a built-in falsifier, since a failed match would
   mean the overgroup picture is wrong even though the cusp-order
   theorem (item 30) stands regardless.
