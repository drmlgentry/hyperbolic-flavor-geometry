...
**Second error found and fixed in the same manuscript, caught while
using it**: §2.2's proof of the surgery law stated the $\Zfive$ torsion
of $H_1(m003)$ is "generated by the class of $a$." Checked directly
(Smith normal form of the relator's exponent vector, `sage`, live):
$r=\texttt{abAAbabbb}$ has exponent vector $(0,5)$ — the relation is
$5[b]=0$, so the torsion is generated by $[b]$, not $[a]$. Fixed in
place, and strengthened the proof to show the full $2\times2$
presentation matrix (relator row $(0,5)$ plus the filling relation from
the verified $\mu=\texttt{ABABB}$, $\lambda=\texttt{ABAbab}$) with
determinant $5(2p+q)$ — an independent re-derivation of the surgery
law's own headline formula from the actual verified peripheral words,
not just a restatement.

## First concrete step on the "charge as topological sector" question
## (`reproduce/m003_canonical_character.py`) — a real structural fact
## found, and an earlier unverified relayed claim corrected

Following through on the speculative research direction discussed this
session (charge as a discrete topological invariant rather than a
continuous geometric one): built the canonical character
$\chi:\pi_1(M_{\rm PMNS})\to H_1(M)=\Zfive\to U(1)$ directly from the
verified presentation and $(-2,3)$ filling relation (the same data
just re-verified above, not assumed).
(`reproduce/m003_canonical_character.py`, sha256
`c11f261bc3f8e9051f66a245f2af7b194aea4d32b8310ed925260078f46bdf1b`,
`EXIT=0`.)

**Finding**: solving the two relations ($5[b]=0$ from the relator,
$[a]+9[b]=0$ from the filling) gives $[a]=[b]$ in $H_1(M)\cong\Zfive$
— confirmed via Sage's own quotient-module machinery (`FreeModule`
quotient), not by hand, after an initial hand/matrix-transform
computation gave a conflicting answer and was discarded in favor of
this more reliable method. Consequence: $\chi(w)$ depends only on the
**total** signed letter count of $w$ (not which of $a,b$ appear) —
there is no further freedom in the construction for this manifold.

**Cross-checked against an already-proven theorem, not just computed
in isolation**: the conjugacy theorem
$(BaBA)\cdot B\cdot(BaBA)^{-1}=Abb^{-1}$ forces $[B]=-[Abb]$ in any
abelianization. Checked: $\chi(B)=4$, $\chi(Abb)=1$,
$4+1\equiv0\pmod5$. Passes. Extended the same check to the three
*other* atlas identities (which are squared-trace, character-variety
facts — nothing a priori requires them to respect homology at all):
**all three also satisfy $[w]=-[w']$** ($\chi(A){=}4,\chi(ABBB){=}1$;
$\chi(AB){=}3,\chi(ABB){=}2$; $\chi(AAb){=}4,\chi(AABB){=}1$). All four
known squared-trace-degeneracy pairs found by the target-free atlas
satisfy this relation. A genuine, verified pattern across all four
currently-known cases — **not** claimed as a proven theorem from first
principles (small sample, mechanism not derived here), but worth
recording as a real, checked fact rather than either asserting or
ignoring it.

**Corrects an earlier, unverified relayed claim**: a large relayed
message earlier this session asserted a homology-class formula
$h(w)=3n_a(w)+n_b(w)\bmod5$ and stated that $B$ and $Abb$ "both lie in
corrected homology class $4\bmod5$." Neither survives contact with the
actual, rigorously-derived character: the correct formula is
$h(w)=n_a(w)+n_b(w)$ (unweighted — a direct consequence of
$[a]=[b]$), and under it $\chi(B)=4$ while $\chi(Abb)=1$ — different
classes, related by $\chi(B)=-\chi(Abb)$ (forced by the conjugacy
theorem), not equal. This was never independently checked earlier this
session; checking it now for the first time as part of building the
character map surfaces the error.

**Explicitly scoped**: this is exploratory theory-building in service
of a speculative physics question (does a canonical, composition-
compatible bulk invariant reproduce anything like electric charge?),
not held to the exact-proof standard of the ITF/character-variety work
elsewhere in this report. It establishes the requested mathematical
...
**Correction (added when the $m006$ analogue was checked, §below):**
of the $25$ collision groups in §5.2, exactly $8$ are **not**
$X_0(m003)$-specific — their trace difference vanishes identically in
$\mathbb{Q}[x,y,z]$ (a generic $\mathrm{SL}_2$ coincidence true for
*any* two-generator representation, verified by direct symbolic check,
not merely by matching under $\Phi$). Only **$17$ of the $25$ groups**
are genuinely tied to $I(X_0)$ (the three certified Stage-4 classes
among them). This does not change the control-test conclusion (ambient
pairs collide under every substitution symmetrically) but corrects the
headline "$58$ classes" figure, which partly reflects generic algebra.
See the $m006$ section below for the full classification and the
cross-check that confirmed this.
...
### Exact certificates for all 28 $m006$ collision groups, plus two sharper structural facts

Reproduce: `reproduce/m006_collision_certificates.py` (+ `.log`). sha256
`333c37d56f6140e6109ef1b8f9553412a16fc33bca2fb76e2dc9dfa96a3e75c4`
(script), `502f4598c6859b59c47dcfd114c65c11918e9f02a1bf779bcace2e96e80ed83a`
(log).

**Explicit thrice-punctured-sphere model.** The Möbius map $u=(t+1)/2$
sends $\{-1,1,\infty\}\mapsto\{0,1,\infty\}$ exactly (checked directly:
$u(-1)=0$, $u(1)=1$, $u(\infty)=\infty$), giving the standard model
$$X_0(m006) \cong \mathbb{P}^1\setminus\{0,1,\infty\}.$$

**A cleaner non-isomorphism argument than Euler characteristic.**
$\mathcal{O}(\mathbb{G}_m)^\times/\mathbb{Q}^\times \cong \mathbb{Z}$
(generated by $t$), while
$\mathcal{O}(\mathbb{A}^1\setminus\{-1,1\})^\times/\mathbb{Q}^\times
\cong \mathbb{Z}^2$ (generated by $t-1,\ t+1$). This free rank
(punctures $-1$) is a $\mathbb{Q}$-isomorphism invariant computed
directly from the coordinate ring, so
$X_0(m003)\not\cong X_0(m006)$ over $\mathbb{Q}$ without any base
change to $\mathbb{C}$.

**Certificate classification — an honest correction to the raw
collision counts.** For every one of the $28$ ($m006$) collision
groups found earlier, reducing $\Delta_{w,v}=\tr(w)-\tr(v)$ modulo
$P_{006}=\langle x-z,\ y(z^2-1)-1\rangle$ and recording the explicit
certificate $\Delta_{w,v}=A\cdot(x-z)+B\cdot\bigl(y(z^2-1)-1\bigr)$
reveals that **$8$ of the $28$ groups are not $m006$-specific at
all**: their $\Delta$ vanishes *identically in $\mathbb{Q}[x,y,z]$*,
before any reduction — a generic $\mathrm{SL}_2$ trace coincidence true
for *any* two-generator representation of *any* group. Checked
directly: **the same $8$ pairs also appear in $m003$'s Stage-5
collision list** (since $\tr(w)$ is a manifold-independent polynomial,
an ambient identity for one manifold is automatically ambient for
every manifold). Re-running the identical check on $m003$'s original
$25$ groups finds the same phenomenon there: exactly $8$ of $m003$'s
$25$ groups are purely ambient, leaving **$17$ genuinely
$X_0(m003)$-specific** groups (the three certified Stage-4 universal
classes among them). So the corrected headline counts are:
$$m003:\ 17\ (\text{of }25)\ \text{genuine}, \qquad
  m006:\ 20\ (\text{of }28)\ \text{genuine} = 11\ (\text{generated by }
  x-z\text{ alone}) + 9\ (\text{needing } y(z^2-1)-1\text{ too}).$$
This does not weaken the earlier control-test comparison (ambient
pairs collide under *every* substitution symmetrically, real manifold
or control, so they wash out of a relative comparison) but it does
mean the raw "$58$/$55$ classes" figures partly reflect generic algebra
rather than either manifold's specific geometry — worth stating
precisely rather than leaving implicit.

**The $A/AB$ family is the largest class of genuinely $m006$-specific
identities**, and the cleanest: $11$ of the $20$ genuine groups
(including $\{A,AB\}$ itself) reduce to $\Delta = A\cdot(x-z)$ with
$B=0$ — i.e.\ they are generated by the *single* equation $x-z$, the
literal statement $\tr(a)=\tr(ab)$ that already IS a defining
generator. The other $9$ groups genuinely require the second generator
$y(z^2-1)-1$ as well. This is an exact two-family classification of
...
### Theorem-level result: does the bounded atlas recover $I(X_0)$? Not quite — same answer, same spurious line, for both manifolds

Following the suggested next direction ("prove that $m006$ word traces
recover both component equations"), the exact question was tested
directly: does the ideal generated by *every* genuinely
component-specific atlas collision (bounded word length $\le6$) equal
$I(X_0)$ itself? Reproduce:
`reproduce/m003_m006_atlas_ideal_vs_component_ideal.py` (+ `.log`).
sha256 `a921233dc6bee85331fadcc4759078f6fc233b91dd568bc9a84418d8781bcfc5`
(script), `da5fa84178718db382340cd71a0284be9d3033fa587a64c79f0ab662e5ae15ee`
(log).

**Answer: no, for either manifold — but the failure is identical and
exact.** Let $J_{\mathrm{atlas}}$ denote the ideal generated by all
genuinely component-specific (non-ambient) atlas collision differences.
Computed exactly for both manifolds:
$$\boxed{\,J_{\mathrm{atlas}}(m003) = I(X_0(m003)) \cap I(N), \qquad
        J_{\mathrm{atlas}}(m006) = I(X_0(m006)) \cap I(N)\,}$$
for the **same** line $N = V(x-z,\ y-2)$ — verified by direct
Groebner-basis equality in both cases, not merely a matching guess.
$J_{\mathrm{atlas}} \subsetneq I(X_0)$ properly in both cases. For
$m006$ the mechanism is transparent: $J_{\mathrm{atlas}}(m006) =
\langle x-z,\ (y-2)(yz^2-y-1)\rangle$ — the first generator $x-z$ is
achieved on the nose (it *is* $\tr(A)-\tr(AB)$), while the second
generator of $I(X_0(m006))$ is only reached multiplied by $(y-2)$.

**This is not a new phenomenon invented for $m006$ — it is the exact
same line found in $m003$'s original Stage-4 ambient decomposition**
($M=V(y-2,x-z)$, §4.4 above), now shown to be the *complete* story for
the full bounded atlas (not just the three originally-known
identities) on **both** manifolds. A natural, not-yet-proved
explanation: $N$ requires $y=\tr(b)=2$, forcing the Fricke matrix $B$
to be parabolic (trace $2$, repeated eigenvalue $1$) — a degenerate
locus where unipotent matrix powers ($B^n = I+n(B-I)$) plausibly
produce generic word-trace coincidences independent of either
manifold's specific relator. Not verified beyond these two manifolds;
flagged as the natural next question rather than claimed as a general
theorem.

**Precise, correct statement of the "recovers both equations" claim:**
the bounded atlas's word-trace algebra does not recover $I(X_0)$ on
the nose for either manifold, but it recovers $X_0$ together with
exactly one universal spurious line $N$ — shared across both
manifolds — rather than $I(X_0)$ itself. That is a sharper and more
informative fact than either "yes, it recovers both equations" or "no,
it doesn't": the failure mode is itself an exact, reproducible,
cross-manifold structure.

### $N$ is genuinely ambient (proved directly, no third manifold needed); $J$ is radical; the "minimal pair" question is closed

Two follow-up questions were raised after the $J_{\mathrm{atlas}} =
I(X_0)\cap I(N)$ finding: is $J$ radical, and is $N$ truly
manifold-independent or a coincidence of these two manifolds
(proposed test: try a third manifold)? Both are answered directly and
more decisively than either proposal, using only the trace-polynomial
algebra already in hand. Reproduce:
`reproduce/m003_m006_universal_line_N.py` (+ `.log`). sha256
`b644705987de265730a90419974682c94c189bc535413f078b4f4309792642a8`
(script), `320133f4a5400a78e54b4b267cb26e058cd9fe2a5112c3395d1b6b8e2736fc0d`
(log).

**$J$ is radical — a one-line argument, not a new computation.** Both
$I(X_0(M))$ and $I(N)=\langle x-z,\ y-2\rangle$ have integral-domain
quotient rings (established already: $\mathbb{Q}[t,t^{-1}]$ or
$\mathbb{Q}[t,(t{-}1)^{-1},(t{+}1)^{-1}]$ for $I(X_0)$; $\mathbb{Q}[t]$
for $I(N)$), hence both are prime; $X_0\neq N$ (a curve vs.\ a line of
different type) in both cases. The intersection of two distinct prime
ideals is automatically radical. No Gr\"obner computation improves on
this.

**$N$ is genuinely ambient — proved directly, not inferred from a
third manifold.** Every one of the $46$ known genuine (non-ambient)
atlas collision differences, from the *complete* scans of both $m003$
and $m006$, vanishes identically on $N=V(x-z,y-2)$ — checked as
$\Delta(x,2,x)=0$ as a polynomial identity in $x$ alone, with **no**
manifold relation imposed at all. This is stronger than a third-
manifold test: it shows *why* $N$ must appear for any manifold whose
bare Riley variety happens to select these particular word pairs,
rather than adding one more (still inconclusive) positive example.
This is **not** because $N$-vanishing is trivial: checked across the
**full** $\binom{99}{2}=4851$ pairwise word-trace differences in the
atlas, only $1028$ ($\approx21.2\%$) vanish ambiently on $N$ — a
genuine, nontrivial, well-defined combinatorial subset, within which
both manifolds' entire genuine-collision sets happen to sit.

**Consequence: the proposed "minimal pair" construction is
algebraically impossible within this word-length bound, not merely
unfound.** A follow-up suggestion asked whether two explicit atlas
differences $\Delta_1=x-z$, $\Delta_2$ (with a *unit* coefficient on
$y(z^2-1)-1$) might already generate the full $P_{006}=I(X_0(m006))$.
Since **every** genuine bounded-length ($\le6$) collision difference
lies in $I(N)$ (previous paragraph), any ideal generated by a finite
subset of them is contained in $I(X_0)\cap I(N) \subsetneq I(X_0)$ —
so **no** finite collection of atlas collisions at this word length can
ever generate the full component ideal, for either manifold. This
closes the question by necessity rather than by an exhausted search.

**Correction to the scope of this claim** (the stated proof only
supports this much): a genuinely component-generating pair *cannot
consist solely of the tested bounded-atlas ($\le6$) collision
differences*. It does **not** follow that longer words would escape
$I(N)$ — that is a separate, open question, sharpened and partly
answered in the next entry below.

### $N$-vanishing has an exact combinatorial characterization — a proven lemma, not an empirical count

Reproduce: `reproduce/m003_m006_N_exponent_lemma.py` (+ `.log`). sha256
`c68703a1d6448097210728e86cae80c12ebb8069b6cb32c647027d233c02ad04`
(script), `4ccef550498aa8165ef879708f376cf14e0822d549a8fd11145e60958d683f36`
(log).

**Lemma (proved for words of every length, not checked only on the
$99$-word atlas).** For any two freely reduced words $w,v$ in
$\{a,A,b,B\}$,
$$\Delta_{w,v}\big|_N = 0 \iff |e_a(w)| = |e_a(v)|,$$
where $e_a$ is the total signed $a$-exponent.

**Proof.** $\tr(w)$ is given, for every word, by a universal Fricke
polynomial $P_w(x,y,z)$ — independent of which representation realizes
a given $(x,y,z)$, by the Cayley–Hamilton recursion alone. So
$P_w(x,2,x)$ can be computed via *any* representation landing there;
take $\rho: a\mapsto(\text{a matrix of trace }x),\ b\mapsto I$. This
$\rho$ has $y=\tr\rho(b)=2$, $z=\tr\rho(ab)=\tr\rho(a)=x$ — exactly a
point of $N$ — and every $b,B$ letter contributes the identity, so
$\rho(w)=\rho(a)^{e_a(w)}$ for *every* word $w$, giving
$P_w(x,2,x)=\tr(\rho(a)^{e_a(w)})=s_{|e_a(w)|}(x)$, the standard
$\mathrm{SL}_2$ trace-power sequence ($s_0=2,\ s_1=x,\ s_n=xs_{n-1}
-s_{n-2}$). The $s_n$ have strictly increasing degree $n$, hence are
pairwise distinct, so $\Delta_{w,v}|_N=s_{|e_a(w)|}-s_{|e_a(v)|}=0$
iff $|e_a(w)|=|e_a(v)|$. $\blacksquare$

Confirmed exactly: (a) $\tr(w)|_N=s_{|e_a(w)|}$ verified directly for
all $99$ atlas words, $99/99$; (b) the $1028$ figure is **independently
re-derived** from pure combinatorics — summing $\binom{c}{2}$ over the
$|e_a|$-value multiplicities ($\{0{:}15,\,1{:}31,\,2{:}22,\,3{:}20,
\,4{:}9,\,5{:}2\}$) gives exactly $1028$, matching the direct
$\binom{99}{2}$ computation; (c) a light, non-exhaustive spot-check at
word lengths $7$–$8$ confirms the closed form continues to hold there.

**This explains, not just documents, why $N$ appeared independently in
both manifolds':** bounded-length collisions happen to preferentially
pair words with matching total $a$-exponent, and *any* such pair is
automatically $N$-vanishing by this lemma — with no reference to
either relator.

**The sharpened open question this leaves** (replacing the earlier,
overstated "needs length $>6$" claim): does *every* genuine
$X_0$-collision ($\Delta\in I(X_0)$, $\Delta\neq0$), at *any* word
length, necessarily satisfy $|e_a(w)|=|e_a(v)|$ — i.e.\ is
$$\ker(\text{word-pair trace equalities on }X_0) \subseteq I(N)$$
a permanent structural fact, not a bounded-atlas artifact? **Not
resolved here.** If true in general, $N$ would be shown to be an
unavoidable obstruction to recovering $I(X_0)$ from word-trace
equalities at *any* length — a materially deeper theorem than anything
established so far in this line. What would resolve it: either a
...
$x^2+y-1$ alone. **[CORRECTED in the next section — the m006 comparison originally
written here was wrong: it used $9/28\approx32\%$, whose denominator
includes the $8$ ambient groups (correct: $9/20=45\%$ of genuine groups),
and it compared a group-level atlas count against this pair-level
length-$\le10$ count. Original text struck: "a sharp quantitative contrast
with m006 ($9/28\approx 32\%$)". See the like-for-like table below.]**
...
### Like-for-like m003/m006 census (correction) and exact stabilization of the collision ideal $J_L$

**Correction of the previous section's m006 comparison.** The "$99.2\%$
versus $32\%$" contrast was not valid: (i) the m006 figure $9/28$ divided
by all $28$ collision groups including the $8$ ambient ones — the correct
share of genuine groups is $9/20=45\%$; (ii) it compared m006's
*group-level, 99-word-atlas* count with m003's *pair-level, length-$\le10$*
count. Recomputed with one identical pipeline for both manifolds
(`reproduce/m003_m006_like_for_like_trichotomy.py`, sha256
`b10d58e9dcbff7ac50663dd5411457453316ac9bbe992307ca996f748b5c1bc3`;
log `3444fb5dfff851a6c6c18996f3b3830ea50cd7a3dd2e8d970b4c1a5c40f4abb5`):

| manifold | $L$ | groups | ambient | genuine | genuine star pairs | pair-level needs-both | group-level needs-both |
|---|---|---|---|---|---|---|---|
| m003 | 6  | 25   | 8   | 17  | 23   | 100.0% | 17/17 |
| m006 | 6  | 28   | 8   | 20  | 26   | 42.3%  | 9/20 (45%) |
| m003 | 10 | 1087 | 546 | 541 | 1316 | 99.2%  | 538/541 (99.4%) |
| m006 | 10 | 1057 | 454 | 603 | 1429 | 77.3%  | 485/603 (80.4%) |

The $L\le6$ m006 row reproduces the earlier $8/11/9$ split exactly. At
$L\le10$, m006 has $324$ pure-$(x-z)$ pairs ($22.7\%$), **zero**
pure-$(yz^2-y-1)$ pairs, and $1105$ needs-both; m003 has $4$/$6$/$1306$.
So a contrast persists ($99.2\%$ vs $77.3\%$ pair-level) but it is much
smaller than first reported, and it *shrinks* with length for m006
($42\%\to77\%$), so the atlas-scale gap overstates the longer-word gap.
No explanation of the residual difference is claimed. Also: all
$1429$ m006 genuine $\Delta$'s vanish on $N$ (the conjecture analogue for
m006, $0$ failures through length $10$).

**Exact stabilization of the collision ideal.** Define
$J_L=\langle\Delta_{w,v}:T_w=T_v,\ \Delta_{w,v}\ne0,\ |w|,|v|\le L\rangle$
(generated by star pairs; within an ambient class $\Delta=0$ and every
cross-class $\Delta$ is a difference of star generators) and
$K=P_0\cap I(N)$, computed independently by elimination.
Reproduce: `reproduce/m003_J10_vs_intersection.py` (sha256
`1dc29326b580890536d1a07946998741ff19b3e969d49aed58ab6600f7fbbb8b`, log
`34baf3af1c2dc42b648852004042cfb8938fe4920adc6d79cc7cba5b2fbb053d`) and
`reproduce/m006_J10_vs_intersection.py` (sha256
`ac645e4d8eb4cc4d2b451285bebe5a731f191f491f40769b6c9f1430d7008cfc`, log
`678e718c11f10e66cfad49f4f770ff9a9eda87af0e8c1e29794cc22a515186f6`).

- **m003**: $K=\langle x^2-xz+y-2,\ yz-x-z\rangle$. Every one of the $1316$
  star $\Delta$'s ($1113$ distinct polynomials) at $L\le10$ vanishes on
  $N$ and reduces to $0$ mod $\mathrm{GB}(K)$: $J_{10}\subseteq K$. And
  $J_L=K$ exactly (reduced Gröbner bases equal) already at **$L=5$**, from
  only $9$ distinct $\Delta$'s ($J_3,J_4\subsetneq K$).
- **m006**: $K=\langle x-z,\ (y-2)(yz^2-y-1)\rangle$; $J_{10}\subseteq K$ for all
  $1429$ star $\Delta$'s ($1179$ distinct), and $J_L=K$ from **$L=4$**.

Concrete generators (m003): $\mathtt{AB}$ vs $\mathtt{ABB}$ gives
$\Delta=x-yz+z$ (length 3); $\mathtt{AAb}$ vs $\mathtt{AABB}$ gives
$(y+1)(x^2-xz+y-2)$ (length 4); $\mathtt{AAbb}$ vs $\mathtt{AABBB}$ gives
$(y^2+y-1)(x^2-xz+y-2)$ (length 5). Since $y^2+y-1=y(y+1)-1$, the
polynomials $y+1$ and $y^2+y-1$ are coprime and the second generator
$x^2-xz+y-2$ falls out by a Euclidean combination.

**What this does and does not say (stated carefully).** Because
$J_L\supseteq J_5=K$ for $L\ge5$ (monotone) and every genuine collision
$\Delta\in P_0=I(X_0)$ ($P_0$ is the kernel of $\Phi$, hence prime), the
containment $J_{10}\subseteq K$ is *exactly* the length-$\le10$ case of the
open conjecture, and the reverse containment is the earlier atlas-level fact
by monotonicity — so no independent evidence beyond the conjecture is
gained. What is gained is a reformulation: the conjecture is **equivalent
to** $J_\infty=J_5=K$, i.e. *no new generators of the collision-relation
ideal appear beyond length $5$* (m003; $4$ for m006). Empirically true
through length $10$ for both manifolds ($V(J_{10})=X_0\cup N$); unproven
